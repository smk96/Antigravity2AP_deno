                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <div class="text-xs text-gray-400 mt-1">
                                Project ID: \${acc.token.project_id ? '<span class="text-green-400">已获取</span>' : '<span class="text-red-400">未获取</span>'}
                                <span class="mx-2">|</span>
                                <span title="\${new Date(acc.token.expiry_timestamp * 1000).toLocaleString()}">Token 过期: \${formatTimeAgo(acc.token.expiry_timestamp * 1000)}</span>
                            </div>
                        </div>
            ...
            container.innerHTML = accounts.map(acc => \`
                <div class="bg-gray-800/50 rounded-lg p-4 border border-gray-700 flex justify-between items-center">
                    <div>
                        ...
                        <div class="text-xs text-gray-400 mt-1">
                            Account ID: \${acc.token.account_id ? '<span class="text-green-400">已获取</span>' : '<span class="text-red-400">未获取</span>'}
                            <span class="mx-2">|</span>
                            <span title="\${new Date(acc.token.expiry_timestamp * 1000).toLocaleString()}">Token 过期: \${formatTimeAgo(acc.token.expiry_timestamp * 1000)}</span>
                        </div>
                    </div>
