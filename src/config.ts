...
const isDenoDeployed = Deno.env.get("DENO_DEPLOYMENT_ID") !== undefined;
...
export async function loadConfig(configPath?: string): Promise<AppConfig> {
  ...
  // 从配置文件加载（如果存在且不在 Deno Deploy 环境）
  if (configPath && !isDenoDeployed) {
    try {
