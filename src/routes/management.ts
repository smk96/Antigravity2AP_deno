...
export function createManagementRoutes(
  ...
  app.get("/auth/antigravity/login", (c) => {
    const { url, state } = antigravityAuth.generateAuthUrl();
  ...
  app.get("/auth/codex/login", async (c) => {
    ...
    const { url, state } = codexAuth.generateAuthUrl(pkceCodes);
  ...
  app.get("/auth/antigravity/callback", async (c) => {
    try {
      ...
      const tokenResponse = await antigravityAuth.exchangeCode(code);
      ...
      // 尝试获取project_id
      try {
        const projectId = await antigravityAuth.fetchProjectId(tokenResponse.access_token);
      ...
      log("info", `Added Antigravity account: ${account.email}`);
  ...
  app.post("/auth/codex/callback", async (c) => {
    try {
      ...
      const tokenResponse = await codexAuth.exchangeCode(code, pkceCodes);
...
function sanitizeAccount(account: AntigravityAccount): Record<string, unknown> {
  return {
    ...
    tokenExpiry: new Date(account.token.expiry_timestamp).toISOString(),
    isExpired: Date.now() > account.token.expiry_timestamp,
